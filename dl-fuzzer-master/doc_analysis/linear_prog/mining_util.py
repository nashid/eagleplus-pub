...
def constr_empty(constr):
    # return True is a row in csv file has no IR label
    for cat in constr:
        if constr[cat]:
            return False
    return True

def parse_ir(row, constr_cols):
    # parse IR from  csv row. 
    # constr_cols: the columns to consider
    
    
    constr_row = init_ir_record(constr_cols)
    nulless = row.isnull()
...
def get_tree_size(tree):    
    if isinstance(tree, str):
        return len([x for x in tree.split() if x!='-1'])
    elif isinstance(tree, list):
        return len([x for x in tree if x!='-1'])
    else:
        assert False
