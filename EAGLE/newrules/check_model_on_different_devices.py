...
def single_rank_execution(
    ...
    try:
        ...
        for key, value in state_dict.items():
            print(rank, key, value.shape)
        path = os.path.join(model_saving_path[:-14], f"model_rank_{rank}")
        print(path)
...
# test run of an example model with some sample input data
def test_run_torchvision_model():
    os.environ["MASTER_ADDR"] = "localhost"
    os.environ["MASTER_PORT"] = "29520"

    tmp_dir = "./torchvision_model"
    log_file = "./log.txt"
    if not os.path.exists(log_file):
        with open(log_file, "w") as f:
            f.write("")
    ...
    test_x = [torch.rand(1, 3, 224, 224).to("cpu")]
...
def compare_model(str):
    ...
    model_0_path = os.path.join(str+"_model", "model_rank_0")
    model_1_path = os.path.join(str+"_model", "model_rank_1")
    model_0_sd= torch.load(model_0_path)
    print(model_0_sd)
    rank = 1
    torch.distributed.init_process_group(rank=rank, world_size=world_size, backend=backend)
    model_1_sd= torch.load(model_1_path)

    match = True
    for key, value in model_0_sd.items():
        if key in model_1_sd:
            if not torch.allclose(value, model_1_sd[key]):
                match = False
                break
        else:
            match = False
    ...
    if match:
        print("{} model has same models on different ranks".format(str))
    else:
        print("{} model has different models on different ranks".format(str))
