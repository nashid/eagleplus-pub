...
import util
from constant import MAX_NUM_DIM, MAX_DIM
...
class Param:
    ...
    def __init__(self, name, spec, dtype_map, required=True):
        # flags
        ...
        self.can_disobey = self.can_disobey_ndim or self.can_disobey_dtype or self.can_disobey_shape
        if self.choices:
    ...
    def gen(self, obey, default_ndims, dtype_map, data_construct, chosen_np_dtype=None):
        """
        This is the generator function for each parameter
        :return: the generated parameter
        """
        if self.choices:  # no need to generate new value & same for both obey and disobey
            data = np.random.choice(self.choices)  # in string, need cast
            to_type = util.infer_type_from_str(data)
            data = util.try_cast(data, to_type)
            return data
        ...
        pick_dtype, pick_ndim = self._pick_dtype_ndim(obey, default_ndims, dtype_map, chosen_np_dtype)
        ...
        data = self._gen_var_from_shape(gen_shape, pick_dtype, dtype_map)
        data = self.convert_structure(data, pick_ndim)
        if data_construct is not None:
            data = self.convert_to_tensor(data, data_construct)
        self.data = data
    ...
    def convert_structure(self, data, pick_ndim):
        ...
        for each_spec in structure_spec:
            if 'list' in each_spec and util.is_iterable(data):  # we simply convert the given data to a list
                ...
                return tuple(data)  # simply convert the given data to a tuple
        return data
    ...
    def _gen_var_from_shape(self, shape, np_dtype, dtype_map):
        # int/uint
        m = re.search('^int([0-9]*)|^uint([0-9]*)', np_dtype)
        if m:
            new_val = self._gen_int(m.group(1), np_dtype, shape)
            return new_val

        # float
        m = re.search('^float([0-9]*)', np_dtype)
        if m:
            new_val = self._gen_float(m.group(1), np_dtype, shape)
            return new_val

        # complex
        m = re.search('^complex([0-9]*)', np_dtype)
        if m:
            new_val = self._gen_complex(m.group(1), np_dtype, shape)
            return new_val
    ...
    def _gen_int(self, n_bit, np_dtype, shape):
        """
        ...
        if not self.range_choices:  # no range spec
            low = default_low
            high = default_high
        else:
            range_spec = self._pick_range_spec(np_dtype, self.range_choices)
            low, high = self._get_low_high_from_range(np_dtype, default_low, default_high)
            if low == high:
                if range_spec.lower_bound_inclusive and range_spec.upper_bound_inclusive:
                    return getattr(np, np_dtype)(low) * np.ones(size)
                util.error('invalid range for "%s" (low == high): %s' % (self.name, range_spec))
            elif low > high:
                util.error('invalid range for "%s" (low > high): %s' % (self.name, range_spec))
    ...
    def _get_low_high_from_range(self, np_dtype, default_low, default_high):
    ...
    def _gen_float(self, n_bits, np_dtype, shape):
    ...
    def _gen_complex(self, n_bit, np_dtype, shape):
        if n_bit.isnumeric():
        ...
        real_part = self._gen_float(half_n_bit, float_dtype, shape)
        imagine_part = self._gen_float(half_n_bit, float_dtype, shape)
        if len(shape) == 0:  # 0D
