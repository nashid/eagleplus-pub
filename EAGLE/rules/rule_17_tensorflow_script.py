...
def test_rule_cpuvsgpu(input, target_fun, log_file):
        ...
        with tf.device('/CPU:0'):
            ...
            for key in input:
                if isinstance(input[key], np.ndarray) or isinstance(input[key], list):
                    inputs_cpu[key] = tf.constant(input[key])
                else:
                    inputs_cpu[key] = input[key]
            output_1 = target_fun(**inputs_cpu)
            # print(input_list)
    except:
    ...
    try:
        ...
        for key in input:
            if isinstance(input[key], np.ndarray) or isinstance(input[key], list):
                inputs_gpu[key] = tf.constant(input[key])
            else:
                inputs_gpu[key] = input[key]

        output_2 = target_fun(**inputs_gpu)
